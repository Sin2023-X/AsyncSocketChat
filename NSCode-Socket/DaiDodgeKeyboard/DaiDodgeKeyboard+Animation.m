...
//    CGRect objectLeftBottom = [currentFirstResponder convertRect:CGRectMake(0, currentFirstResponder.frame.size.height, [self observerView].frame.size.width, [self observerView].frame.size.height - currentKeyboardRect.size.height) toView:[self observerView]];
...
//        objectLeftBottom.origin.y += textView.contentOffset.y;
//    }
    
//    float shiftHeight = objectLeftBottom.y - currentKeyboardRect.origin.y;
    
    if (currentKeyboardRect.size.height > 0) {
        [UIView animateWithDuration:[self keyboardAnimationDutation]
                         animations:^{
                             ...
                             newFrame.size.height = [UIScreen mainScreen].bounds.size.height - 64 - currentKeyboardRect.size.height;                             
